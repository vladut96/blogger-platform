...
let UsersRepository = class UsersRepository {
    ...
    async updateConfirmationStatus(email, status) {
        await this.userModel.updateOne({ email }, { $set: { 'emailConfirmation.isConfirmed': status } });
    }
    async updateConfirmationCode(email, newCode, expirationDate) {
        const result = await this.userModel.updateOne({ email }, {
            $set: {
    ...
    async setPasswordRecoveryCode(dto) {
        const result = await this.userModel.updateOne({ email: dto.email }, {
            $set: {
    ...
    async updateUserPassword(email, newPasswordHash) {
        const result = await this.userModel.updateOne({ email }, { $set: { passwordHash: newPasswordHash } });
        return result.modifiedCount === 1;
    }
    async save(newUser) {
